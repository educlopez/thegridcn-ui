...
const intensityIndexById = new Map(
  tronIntensities.map((i, idx) => [i.id, idx])
);
    ...
    <div className="flex gap-1 rounded-lg border border-border bg-card p-1">
      {tronIntensities.map((i) => (
        <button
          ...
          className={cn(
            "relative rounded-md px-2 py-1 font-medium text-xs transition-all duration-300",
            tronIntensity === i.id
  ...
  return (
    <div className="flex flex-col gap-2">
      <div className="flex items-center justify-between">
        <span className="font-medium text-sm">Tron Intensity</span>
        <span className="text-primary text-sm">{currentIntensity?.name}</span>
      </div>
    ...
    >
      <span
        className={cn(
          ...
          tronIntensity === "none" ? "text-muted-foreground" : "text-primary"
        )}
      >
        {intensityIcons[tronIntensity]}
      </span>
      <span className="font-medium text-sm">{currentIntensity?.name}</span>
    </button>
  );
}
