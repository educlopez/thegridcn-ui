    ...
    >
      ...
      <div className="relative overflow-x-auto">
        <table className="w-full border-collapse">
          ...
          <thead>
            <tr>
              <th className="border-primary/20 border-b px-4 py-3 text-left font-mono text-[10px] text-foreground/40 uppercase tracking-widest">
                Feature
              </th>
              {columns.map((col, i) => (
                <th
                  key={i}
                  className={cn(
                    "border-primary/20 border-b border-l px-4 py-3 text-center font-mono text-[10px] uppercase tracking-widest",
                    col.highlighted
          ...
          <tbody>
            {features.map((feature, ri) => (
              ...
              >
                <td className="border-primary/10 border-b px-4 py-2.5 font-mono text-foreground/70 text-xs">
                  {feature.name}
                </td>
                {feature.values.map((val, ci) => (
                  <td
                    key={ci}
                    className={cn(
                      "border-primary/10 border-b border-l px-4 py-2.5 text-center font-mono text-xs",
