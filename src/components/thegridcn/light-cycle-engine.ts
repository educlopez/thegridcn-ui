...
const PERSONALITY_ORDER: AIPersonality[] = [
  "hunter",
  "waller",
  "survivor",
  "cutoff",
...
export class LightCycleEngine {
  ...
  private readonly initialConfigs: BikeConfig[];
  bikes: Bike[];
  phase: GamePhase;
  winner: number | null;
  private readonly occupied: Set<string>;
  private readonly aiProfiles: AIProfile[];
  // Shared AI state computed once per tick
  private playerReachable: Set<string> = new Set();

  constructor(gridSize: number, configs: BikeConfig[], aiLevel = 1) {
  ...
  setBikeDirection(bikeIndex: number, dir: Direction) {
    ...
    if (OPPOSITE[bike.direction] === dir) {
      return;
    }
  ...
  private computePlayerTerritory() {
    ...
    while (queue.length > 0 && this.playerReachable.size < limit) {
      ...
      for (const d of ALL_DIRECTIONS) {
        ...
        if (this.playerReachable.has(key) || this.occupied.has(key)) {
          continue;
        }
        this.playerReachable.add(key);
        if (this.playerReachable.size >= limit) {
          break;
        }
        queue.push([nx, ny]);
      }
  ...
  private runAI(bikeIndex: number) {
    ...
    if (
      profile.randomSkipChance > 0 &&
      Math.random() < profile.randomSkipChance &&
      this.isDirectionSafe(bike, currentDir)
    ) {
      return;
    }
    ...
    const [player] = this.bikes;
    const playerDist =
      Math.abs(bike.x - player.x) + Math.abs(bike.y - player.y);
    ...
    for (const d of safeDirs) {
      ...
      if (
        profile.territoryCutBonus > 1.0 &&
        player.alive &&
        this.playerReachable.has(cellKey(nx, ny))
      ) {
      ...
      if (
        profile.targetRange > 0 &&
        player.alive &&
        playerDist <= profile.targetRange
      ) {
        ...
        const distBefore =
          Math.abs(bike.x - targetX) + Math.abs(bike.y - targetY);
      ...
      if (profile.parallelWallBonus > 1.0 && player.alive && playerDist <= 15) {
        const isParallel =
          d === player.direction || d === OPPOSITE[player.direction];
      ...
      if (profile.wallHugBonus > 1.0) {
        ...
        if (adjacentWalls === 1) {
          ...
          score *=
            profile.personality === "waller"
              ? profile.wallHugBonus * 0.95
              : 0.85;
        }
      }
      ...
      if (profile.rivalAvoidRange > 0) {
        for (let r = 1; r < this.bikes.length; r++) {
          if (r === bikeIndex || !this.bikes[r].alive) {
            continue;
          }
          const rival = this.bikes[r];
          const rivalDist =
            Math.abs(bike.x - rival.x) + Math.abs(bike.y - rival.y);
      ...
      if (profile.trailLookahead > 1) {
        ...
        for (let step = 0; step < profile.trailLookahead; step++) {
          ...
          if (this.occupied.has(cellKey(lookX, lookY))) {
            break;
          }
          clearAhead++;
        }
    ...
    if (safeDirs.includes(currentDir)) {
      ...
      if (
        profile.territoryCutBonus > 1.0 &&
        this.bikes[0].alive &&
        this.playerReachable.has(cellKey(cnx, cny))
      ) {
  ...
  private isDirectionSafe(bike: Bike, dir: Direction): boolean {
    ...
    if (this.occupied.has(cellKey(nx, ny))) {
      return false;
    }
    // Check if another alive bike is at that position
    for (const other of this.bikes) {
      if (other !== bike && other.alive && other.x === nx && other.y === ny) {
        return false;
      }
    }
    ...
    for (const other of this.bikes) {
      if (other !== bike && other.alive) {
        ...
        if (other.x + od.dx === nx && other.y + od.dy === ny) {
          return false;
        }
      }
    }
    ...
    for (const d of ALL_DIRECTIONS) {
      ...
      const ex = nx + dd.dx;
      const ey = ny + dd.dy;
      if (ex < 0 || ex >= this.gridSize || ey < 0 || ey >= this.gridSize) {
        continue;
      }
      if (this.occupied.has(cellKey(ex, ey))) {
        continue;
      }
      hasExit = true;
      break;
  ...
  private countReachable(
    ...
    while (queue.length > 0 && visited.size < limit) {
      ...
      for (const d of ALL_DIRECTIONS) {
        ...
        const key = cellKey(nx, ny);
        if (visited.has(key) || this.occupied.has(key)) {
          continue;
        }
        visited.add(key);
