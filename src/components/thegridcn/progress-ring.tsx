...
const sizeConfig = {
  lg: { dim: 160, fontSize: 28, labelSize: "text-xs", stroke: 6, tickR: 4 },
  md: { dim: 120, fontSize: 20, labelSize: "text-[10px]", stroke: 5, tickR: 3 },
  sm: { dim: 80, fontSize: 14, labelSize: "text-[8px]", stroke: 4, tickR: 2 },
};
...
export function ProgressRing({
  ...
  }, [clamped, animated]); // eslint-disable-line react-hooks/exhaustive-deps
  ...
  return (
    ...
    >
      <svg
        aria-hidden="true"
        width={config.dim + config.tickR * 4 + 8}
        height={config.dim + config.tickR * 4 + 8}
      >
        <defs>
          <filter id={filterId}>
        ...
        <g
          transform={`translate(${config.tickR * 2 + 4}, ${config.tickR * 2 + 4})`}
        >
          ...
          {showValue ? (
            <text
              ...
              className={cn("font-bold font-mono", variantText[variant])}
              fill="currentColor"
