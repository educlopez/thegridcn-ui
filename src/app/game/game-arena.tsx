...
export function GameArena() {
  ...
  React.useEffect(() => {
    if (!(derezzed || showReadyOverlay)) {
      return;
    }

    function handleKeyDown(e: KeyboardEvent) {
      if (e.key === "Enter" && derezzed) {
        handleRestart();
      }
    }
      ...
      <div className="relative inline-block">
        ...
        {showReadyOverlay ? (
          <div className="absolute inset-0 z-10 flex flex-col items-center justify-center gap-4 overflow-y-auto p-6">
            <AnomalyBanner title="LIGHT CYCLE" subtitle="ENTER THE GRID" />

            {/* Difficulty selector */}
            <div className="w-full max-w-[400px]">
              <div className="mb-1.5 text-center font-mono text-[9px] text-muted-foreground/60 tracking-[0.2em]">
                SELECT DIFFICULTY
              </div>
              ...
              <div className="mt-1 text-center font-mono text-[9px] text-muted-foreground/60 tracking-[0.2em]">
            ...
            <span className="font-mono text-[9px] text-amber-500/40 tracking-widest">
        ...
        {derezzed ? (
          <div className="absolute inset-0 z-10 flex flex-col items-center justify-center gap-6 p-8">
            <AnomalyBanner title="DEREZZED" subtitle="PROGRAM TERMINATED" />
            <button
              type="button"
              onClick={handleRestart}
              className="border border-amber-500/50 bg-amber-500/10 px-6 py-2.5 font-mono text-amber-500 text-xs tracking-[0.3em] transition-all hover:border-amber-500 hover:bg-amber-500/20"
            >
              RESTART CYCLE
            </button>
            <span className="font-mono text-[9px] text-amber-500/40 tracking-widest">
