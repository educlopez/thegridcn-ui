...
async function enhanceRegistryFiles() {
  ...
  try {
    ...
    if (!existsSync(publicRPath)) {
      console.warn(
        "⚠️  public/r directory not found. Run 'pnpm registry:build' first."
      );
      return;
    }
    ...
    for (const file of jsonFiles) {
      ...
      if (registryEntry) {
        ...
        const needsRegistryDeps =
          registryEntry.registryDependencies &&
          registryEntry.registryDependencies.length > 0;

        // Only update if we need to add missing dependencies
        if (
          (needsDeps && !hasDeps) ||
          (needsRegistryDeps && !hasRegistryDeps)
        ) {
          const enhanced = {
            ...componentData,
            ...(registryEntry.dependencies &&
              registryEntry.dependencies.length > 0 && {
